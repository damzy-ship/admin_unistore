...
const Merchants: React.FC = () => {
  ...
  const { merchants, merchantProducts, loading, error, refetch } = useMerchants(filters)

  const filteredMerchants = merchants.filter(merchant => 
    merchant.brand_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    merchant.full_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    merchant.email?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    merchant.user_id.toLowerCase().includes(searchTerm.toLowerCase())
  )
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div className="px-6 py-4 bg-gray-50 border-t flex items-center justify-between">
          <div className="text-sm text-gray-500">
            Showing <span className="font-medium">1</span> to <span className="font-medium">{filteredMerchants.length}</span> of{' '}
            <span className="font-medium">{merchants.length}</span> merchants
          </div>

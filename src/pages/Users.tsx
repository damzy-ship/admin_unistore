...
import { useUsers, updateUserType, updateUserSchool } from '../hooks/useSupabaseData'
...
const Users: React.FC = () => {
  ...
  const handleVerificationUpdate = (updated?: Partial<UniqueVisitor> | null) => {
    if (updated && selectedUser && selectedUser.id === updated.id) {
      setSelectedUser(Object.assign({}, selectedUser, updated as UniqueVisitor))
    }
    refetch()
  }

  const handleUserTypeChange = async (userId: string, newType: string) => {
    if (newType === 'user' || newType === 'merchant' || newType === 'visitor') {
      if (window.confirm(`Are you sure you want to change this user type to ${newType}? They will be moved to the ${newType === 'user' ? 'Users' : 'Merchants'} list.`)) {
        try {
          await updateUserType(userId, newType as 'user' | 'merchant' | 'visitor')
          refetch()
        } catch (error) {

...
import {
  ...
  fetchHostelsBySchoolId,
  updateHostelMerchantStatus
...
interface VerificationModalProps {
  ...
  user: UniqueVisitor & { is_hostel_merchant?: boolean } | null
  isOpen: boolean
  onClose: () => void
...
const VerificationModal: React.FC<VerificationModalProps> = ({
  ...
  // NEW: Feedback View State
  const [feedbackView, setFeedbackView] = React.useState<{
    type: 'success' | 'error'
    title: string
    message: string
  } | null>(null)
  ...
  const handleVerification = async (status: 'verified' | 'unverified') => {
    ...
    try {
      ...
      setFeedbackView({
        ...
        message: `The user has been successfully ${status === 'verified' ? 'verified' : 'rejected'}. Their status has been updated.`
      })
      ...
      setFeedbackView({
        ...
        message: 'Something went wrong while updating the verification status. Please try again.'
      })
    } finally {
      setLoading(false)
    }
  ...
  const handleHostelUpdate = async () => {
    ...
    try {
      ...
      setFeedbackView({
        ...
        message: 'Hostel merchant configuration has been successfully updated.'
      })
      ...
      setFeedbackView({
        type: 'error',
        title: 'Update Failed',
        message: 'Unable to update hostel merchant configuration. Please check your connection and try again.'
      })
    } finally {
      setLoading(false)
    }
  ...
  const handleFeedbackAction = () => {
    if (feedbackView?.type === 'success') {
      onClose() // Close modal on success done
    } else {
      setFeedbackView(null) // Return to form on error retry
    }
  }
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block w-full max-w-2xl p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-lg">
          ...
          {/* Conditional Rendering: Feedback View vs Form View */}
          {feedbackView ? (
            <StatusFeedback
              ...
              actionLabel={feedbackView.type === 'success' ? 'Done' : 'Try Again'}
            />
          ) : (
            <>
